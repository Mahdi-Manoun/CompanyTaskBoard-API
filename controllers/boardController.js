...
const deleteBoard = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `${board.title} deleted successfully` })
    } catch (error) {
...
const updateBoard = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `Board title updated successfully to ${newTitle} with ${newBackgroundColor} color` })
    } catch (error) {
        console.error(error)
        res.status(500).json({ error: 'Server Error' })
    }
